...
function MainGameCartela({
  ...
  const localGrid = useMemo(
    () => (serverGrid ? null : generateBingoCard(cartelId).grid),
    [cartelId, serverGrid]
  );
        ...
        {isFree ? (
          <Star
            className={`shrink-0 fill-white text-white ${
              stacked
                ? 'h-[clamp(11px,2.8vw,15px)] w-[clamp(11px,2.8vw,15px)]'
                : 'h-[clamp(13px,3.2vw,17px)] w-[clamp(13px,3.2vw,17px)]'
            }`}
...
export default memo(MainGameCartela);
